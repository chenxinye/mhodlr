function C = hadd_partial_double(A, B, operator)
% A must be hodlr format, while B must be double format
    ...
    if isempty(A.D) 
        ...
        C(1:m1, 1:n1) = hadd_partial_double(A.A11, B(1:m1, 1:n1), operator);
        C(m1+1:end, n1+1:end) = hadd_partial_double(A.A22, B(m1+1:end, n1+1:end), operator);
