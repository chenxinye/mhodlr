...
%{
    ...
    itype- int
        `0`: Solve XU = B, U is upper triangular matrix.

        `1`: Solve XU = B, U is upper triangular matrix, used for LU factorization.
    ...
    if itype ~= 2 % Solve XU = B where U is upper triangular matrix
        ...
        if isa(B, 'hodlr') | isa(B, 'mphodlr') 
            ...
            if isempty(U.D)
                ...
                U12 = U.U1*U.V2;
                [X.U1, X.V2] = compress_m(htrsu(B.U1*B.V2 - hdot(X.A11, U12, 'dense'), U.A22, 1), U.method, U.threshold);
