...
precision_labels = {'fp32', 'bf16', 'bf16'}; % For CSV column headers
...
for m = 1:length(matrix_names)
    for d = 1:length(depths)
        ...
        title(sprintf('%s, Depth %d (Cholesky)', matrix_names{m}, depths(d)), 'FontSize', 16);
        ...
        plot_filename = sprintf('chol_%s_depth%d_barplot.png', matrix_names{m}, depths(d));
