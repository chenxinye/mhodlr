function A = dense(H, varargin)
%{
    The function is to dense a HODLR format into array format
...
function A = dense_mat(H)
    if ismember('D', properties(H))
        if isempty(H.D) 
            ...
            A(1:su1, 1:sv1) = dense_mat(H.A11);
            A(su1+1:end, sv1+1:end) = dense_mat(H.A22);
        else
            A = H.D;
        end
