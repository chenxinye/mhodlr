...
function C = mhdot_hodlr_helper(A, B, isA_hodlr, isB_hodlr)
    ...
    if isA_hodlr && isB_hodlr
        ...
        if ~isempty(A.D)
            D = mchop(A.D * B.D);
            C = amphodlr(D, 0, A.min_block_size);
            C.vareps = vareps;
            [m, n] = size(D);
            C.shape = [m, n];
        else
    ...
    elseif isA_hodlr
        ...
        if ~isempty(A.D)
            ...
            C.A22 = mhadd_partial_hodlr(mhdot_hodlr_helper(A.A22, B(sv1+1:end, midB+1:end), true, false), ...
    ...
    elseif isB_hodlr
        ...
        if ~isempty(B.D)
            D = mchop(A * B.D);
            C = amphodlr(D, 0, B.min_block_size);
            C.vareps = vareps;
            [m, n] = size(D);
            C.shape = [m, n];
        else
            ...
            % Diagonal blocks
            C.A11 = mhadd_partial_hodlr(mhdot_hodlr_helper(A(1:midA, 1:su1), B.A11, false, true), ...
                                       AU2(1:midA, :) * B.V1, '+', true);
            C.A22 = mhadd_partial_hodlr(mhdot_hodlr_helper(A(midA+1:end, su1+1:end), B.A22, false, true), ...
