...
for m = 1:length(matrix_names)
    for d = 1:length(depths)
        % Read CSV file
        filename = sprintf('inv_%s_depth%d.csv', matrix_names{m}, depths(d));
        ...
        title(sprintf('%s, Depth %d (Inverse)', matrix_names{m}, depths(d)), 'FontSize', 16);
        ...
        % Save plot
        plot_filename = sprintf('inv_%s_depth%d_barplot.png', matrix_names{m}, depths(d));
